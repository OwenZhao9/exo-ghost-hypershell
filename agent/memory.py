...
只有调用方显式给了 node_secret 才会碰远端（EvoMap 官方明文要求不准自动注册）。

**硬规矩：查库会阻塞（sqlite、可能还有网络），绝不能在串口读线程里发生。**
所以这里所有公开方法都只往一个有界队列里放任务，由一个后台线程去做；
...
class GhostMemory:
    ...
    def recall_for_event(self, event: str, k: int = 2) -> bool:
        ...
        q = EVENT_QUERIES.get(event.split(":", 1)[0])
        if not q:
            return False
        self.recall(q, k)
        return True
