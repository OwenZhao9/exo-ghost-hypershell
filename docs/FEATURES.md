...
- **左右独立助力与阻力**：`feat/bilateral-modes-GPT` 的 3D 页为左右腿各提供松劲、助力、阻力及强度设置，可单独应用一侧并保留另一侧当前模式，也可在一条命令中同时应用两侧，包括左助力右阻力等组合。`site/control.js` 只在实时真机、确认安全档、`ARMED`、传感器与状态新鲜、至少 50 Hz 且服务报告 `bilateral_modes=true` 时构造 `policy=bilateral` 命令；旧服务仍可使用原有双腿模式，支持 `split_resist` 的服务继续显示旧独立阻力入口。`runtime/commands.py` 原子切换两侧；`control/policies.py` 对每侧助力增益限为 0.1、阻力增益限为 0.5、单侧力矩限为 0.5 N·m，并沿用死区、低通、缓升及穿戴档限速。`runtime/service.py` 对助力侧单独计入正功预算，安全渐弱不削弱另一侧阻力；腿板掉线、重连或急停清回零策略，不自动恢复。数据来自现有串口帧；网页不打开串口。147 项离线 Python 测试与 9 项站点测试通过，Chrome 本地页面已检查；混合模式真机输出与穿戴效果尚未验证，当前运行的旧服务未启用该能力。
