...
- **穿戴贴合**：`site/data/twin-config.json` 的 `fit` 为 Tripo 外骨骼模型列出腰部和双侧腿部固定件的网格名称、收紧比例及膝上位移；`site/app.js` 在双髋枢轴分组后让腰带向身体收紧，并把双侧固定环收小、抬至下沿高于白膜人体膝关节约 1 厘米。腿部支撑杆的下段平滑跟到新环位，避免支撑杆和腿环脱开；固定环仍随各自髋关节旋转。加载配置时使用 `no-store`，避免本地浏览器缓存旧版配置导致 3D 视图无法启动。1 厘米由当前 GLB 骨骼位置及模型比例换算，未做实体尺寸测量；这些数值只校正本地 3D 模型与白膜人体的视觉叠合，不代表实体绑带松紧，也不发送设备指令。不同身材和实际穿戴位置仍需现场核对。
