...
连续拍照 Demo 由 `product_features/guide/demo.py` 顺序调用独立 Luma 拍照程序，后台线程不占外骨骼串口。`--glasses-bin`、`--glasses-unit` 与 `--guide-demo-upload` 均需显式配置；没有上传开关时只保存本机照片。页面每 2 秒检查照片和演示状态，图像仅从本机 API 读取。EvoMap 返回严格校验的 JSON；低置信度、画面模糊、格式异常或服务失败都不给左/右结论。左/右仅表示照片中看起来较空的一侧，不能作为移动指令。真机桌面验收两张照片的采集分别耗时 11.5 秒、11.1 秒；第一张识别耗时 10.6 秒且结论为“无法判断”。这不是实时视频流，未完成实际导盲验证。
