...
- **验证与未完成**：`tests/test_mobile.py` 覆盖配对、扫码载荷、鉴权、白名单和断线租约；扫码版完整 Python 回归 133 项通过。Xcode 26.4 构建的 `1.0 (1)` 已上传 TestFlight 并分配给只含账户持有人的内部测试群组；扫码版 `1.0 (2)` 已上传，等待 Apple 处理。二维码已用 Mac Vision 识别验证，相机权限拒绝路径已在模拟器检查。仍缺 iPhone 8 实机扫描、真实连接和控制验证。局域网 WebSocket 仍为明文，限可信 Wi-Fi，不支持公网远程控制。具体操作见 [iPhone 工程说明](../ios/GhostMobile/README.md)。
