...
- **页面状态提示**：左右腿独立调节卡片只保留模式、强度和应用按钮；按用户要求不在卡片下方显示控制服务版本提示。按钮禁用条件与命令校验仍保留，旧服务缺少 `bilateral_modes` 时不能发送独立控制命令。
- **行为与入口**：`site/#twin` 新增动力辅助与健身阻力按钮，分别使用 `assist` 增益 0.1、上限 0.5 N·m 和 `resist` 增益 0.3、上限 0.5 N·m。这是产品分支的首次使用低强度预设。页面停止入口只显示“松劲”，连接存在时发送 `zero`；不显示急停按钮，底层急停机制保留。当前模式来自控制服务状态回报，页面加载与断线重连不会自动施力。
...
- **兼容与验证边界**：旧控制进程若未报告 `profile`，实时 3D 仍可显示，但运动按钮禁用。2026-09-24 已在穿戴档做短时阻力与助力测试，确认服务接收命令并下发低幅度力矩；助力体感仍弱，尚无实测电机力矩或可证明有效减负的数据。切换运行服务必须先释放串口，不得让两个控制进程同时连接设备。
...
- **左右独立助力与阻力**：`feat/bilateral-modes-GPT` 的 3D 页为左右腿各提供松劲、助力、阻力及强度设置，可单独应用一侧并保留另一侧当前模式，也可在一条命令中同时应用两侧，包括左助力右阻力等组合。页面移除常驻桌面档勾选框，首次点击运动按钮时按 `table/wearing` 弹出现场确认，确认后重新检查实时真机、`ARMED`、传感器与状态新鲜、至少 50 Hz 且服务报告 `bilateral_modes=true` 才构造 `policy=bilateral` 命令。桌面档只用于未穿戴测试，穿戴测试须由 `--profile wearing` 服务运行并完成穿戴前检查；断线、安全档变化或数据失效会撤销确认。旧服务仍可使用原有双腿模式，支持 `split_resist` 的服务继续显示旧独立阻力入口。`runtime/commands.py` 原子切换两侧；`control/policies.py` 对每侧助力增益限为 0.1、阻力增益限为 0.5、单侧力矩限为 0.5 N·m，并沿用死区、低通、缓慢加载及穿戴档限速。`runtime/service.py` 对助力侧单独计入正功预算，安全渐弱不削弱另一侧阻力；腿板掉线、重连或急停清回零策略，不自动恢复。数据来自现有串口帧；网页不打开串口。148 项离线 Python 测试与 9 项站点测试通过，Chrome 本地页面已检查；当前穿戴档服务已报告 `bilateral_modes=true`，但混合模式真机输出与穿戴效果尚未验证。
...
- **3D 区域用语**：页面操作只写“隐藏/显示人物”“重置站姿”“转身同步”等用户动作；连接、回放、保护状态直接说明当前可见结果，不显示串口、视觉基准、模型后端或原始增益。运动卡片保留功能说明，安全档和控制门槛仍按设备真实状态执行；技术参数、数据来源与验证限制记录在本文档和 `site/README.md`。这次只改文字与当前模式的展示形式，不改变控制命令和设备行为。
- **3D 页面文案**：按用户要求，`site/#twin` 控制区下方不再显示整段模型来源、动作估算和数据边界小字；控制区也不常驻显示安全档和等待设备的提示，仅在用户提交操作后显示结果或错误。连接状态与实时读数仍在实时视图中，运动模式及松劲入口保留；来源、授权、视觉映射和验证限制继续在本功能文档、`site/README.md` 与 `model-sources/` 中维护。
...
- **边界**：网页为静态资源，订阅本机设备 WebSocket；仅在安全门槛满足后发送运动模式命令，连接存在时可发送“松劲”命令。网页不发送急停命令，不调用串口或私人数据库；控制服务和设备层急停保持原有行为。新增功能需对应更新展示内容，同时保留已验证状态与数据来源说明。2026-09-23 验证时控制进程带 `--body real --profile table`，Chrome 显示约 178–179 Hz 的实时双髋读数；设备静置，转身方向与真实穿戴行走尚需现场对照。旧进程未在 WebSocket 状态中标记 `body`，所以网页仍显示“来源未标记”。
- **本机控制连接**：控制服务支持 `--ws-host 127.0.0.1`，使页面的 WebSocket 控制通道仅在本机监听；默认监听行为保持原样。穿戴时应使用 `--profile wearing`，其启动策略为 `zero`。当前助力策略按髋角速度输出，静止时为零；2026-09-24 用户穿戴反馈助力几乎无感时，现场旧桌面档服务的记录显示运动片段力矩中位数约 0.05 N·m、最大约 0.21 N·m。切换穿戴档后，增益 0.2、上限 0.5 N·m 的 20 秒测试在有动作帧中的下发力矩中位数约 0.12 N·m、峰值约 0.27 N·m；用户仍反馈几乎无感。增益 0.6、上限 0.5 N·m 的一次测试几乎没有运动，无法判断体感；随后增益 0.6、上限 0.8 N·m 的短时真机测试在有动作帧中位数约 0.12 N·m、峰值约 0.30 N·m，因穿戴档 600 秒会话上限到期自动停机，最终状态 `TRIPPED`、策略 `zero`、输出 0。用户提出“检测方向后持续 2 N·m”，该算法未实现，也未在穿戴设备上绕过现有助力策略 1.2 N·m 与服务 1.5 N·m 的限幅。这些数值是下发指令，不是实测电机力矩；单纯提高最大力矩不能改变静止时零输出的算法行为。
