...
- **人体模型**：`site/assets/human-tripo-rigged.glb` 由 Tripo 根据白膜参考图生成并自动绑骨，呈现无五官、无发型的自然人体轮廓。参考图从获准使用的 Hypershell 官方穿戴照片提取人体比例与展示风格，未复刻照片真人身份或旧设备；生成记录见 `model-sources/README.md`，来源说明见 `site/assets/HUMAN-LICENSE.md`。`site/app.js` 移除模型贴图并使其呈半透明白色，旋转 180° 修正人物与外骨骼的前后穿戴方向，再按髋骨位置对齐外骨骼，并以同一双髋角度驱动腿骨；可显示/隐藏。人体旋转后的解剖左髋位于模型 `+Z`，因此实体左通道驱动人体左髋及外骨骼配置中的 `right`（`+Z`）组，实体右通道驱动另一侧。这只修正视觉左右对应，不交换页面左右数值，也不改变电机指令。`site/knee.js` 基于双髋相对已记录直立参考的前抬角生成平滑的视觉膝部姿态：直立附近伸直，前抬时驱动白膜左右膝骨骼让小腿向脚跟方向屈曲，最多 135°；依据当前 GLB 骨骼验证了双侧方向，腿部固定件和真机控制不随膝角估算而运动。设备没有膝角、足部或手臂传感器，网页不显示膝角实测值；视觉屈膝不能证明真人实际弯膝。该模型仍为视觉近似，不是官方人体 3D 或精确人体扫描。
