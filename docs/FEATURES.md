...
- **控制门槛**：只在实时模式，且现有控制服务报告 `body=real`、`profile=table|wearing`、`ARMED`、至少 50 Hz、状态与传感器帧新鲜时，才显示可操作的运动按钮。页面移除了常驻桌面档勾选框，首次点击运动按钮时按实际安全档弹出确认；确认后重新检查新鲜数据才发送命令。桌面档仅用于无人穿戴的桌面或支架测试；穿戴测试必须由 `--profile wearing` 服务运行并完成穿戴前检查。断线、状态失效或安全档变化会撤销页面确认，重连不自动施力。历史回放与仿真不开放施力。
...
- **总览展示**：首屏突出项目自有外骨骼模型渲染和直接演示入口，随后用外骨骼、白膜人体两个项目内 3D 模型的多角度渲染展示形态；靠近 3D 区域时自动加载可旋转模型，失败时可手动重试。总览页不引用官网产品宣传图片，渲染资源由 `tools/render_site_models.py` 从 `site/assets/` 中的 GLB 生成。
