...
- **人体模型**：`site/assets/human-tripo-rigged.glb` 由 Tripo 根据白膜参考图生成并自动绑骨，呈现无五官、无发型的自然人体轮廓。参考图从获准使用的 Hypershell 官方穿戴照片提取人体比例与展示风格，未复刻照片真人身份或旧设备；生成记录见 `model-sources/README.md`，来源说明见 `site/assets/HUMAN-LICENSE.md`。`site/app.js` 移除模型贴图并使其呈半透明白色，旋转 180° 修正人物与外骨骼的前后穿戴方向，再按髋骨位置对齐外骨骼，并以同一双髋角度驱动腿骨；可显示/隐藏。人体旋转后的解剖左髋位于模型 `+Z`，因此实体左通道驱动人体左髋及外骨骼配置中的 `right`（`+Z`）组，实体右通道驱动另一侧。这只修正视觉左右对应，不交换页面左右数值，也不改变电机指令。白膜转向后，模型髋关节负旋转代表向前抬腿；`site/app.js` 将其转换为正的抬腿幅度，`site/motion.js` 再根据髋角及角速度估算屈膝，并平滑回伸。膝骨骼正向旋转让小腿朝身后弯曲；视觉屈膝上限从 65° 调到 135°，避免约 100° 的髋部前抬时小腿整体仍朝脚尖。两个方向均由模型骨骼和脚尖几何校验，包括大幅抬腿场景。只改变视觉小腿姿态；设备没有膝角、足部或手臂传感器，屈膝不代表实测动作。双髋角度不变时无法识别已抬腿姿态。该模型仍为视觉近似，不是官方人体 3D 或精确人体扫描。
