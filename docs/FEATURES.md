...
| 眼镜看一看（试验） | `product_features/guide/` | 从指定的 Luma 眼镜照片目录列出 JPEG，并在页面显示最近一张。手动选图仍可请求 EvoMap 描述。`feat/glasses-demo-GPT` 新增可选的连续拍照 Demo：用户在本机页面启动后，逐张拍照、刷新画面并自动请求 EvoMap 比较画面左右两侧；不清楚时显示“无法判断”。每轮最多 10 张，失败 3 次停止，随时可手动停止。没有真人录音时不播放语音，不提供实际导盲或外骨骼动作。 |
...
连续拍照 Demo 由 `product_features/guide/demo.py` 顺序调用独立 Luma 拍照程序，后台线程不占外骨骼串口。`--glasses-bin`、`--glasses-unit` 与 `--guide-demo-upload` 均需显式配置；没有上传开关时只保存本机照片。页面每 2 秒检查照片和演示状态，图像仅从本机 API 读取；拍摄下一张期间保留“最近照片”的上一次判断，拍成新照片后清除旧判断直到新识别完成。EvoMap 返回严格校验的 JSON；低置信度、画面模糊、格式异常或服务失败都不给左/右结论。左/右仅表示照片中看起来较空的一侧，不能作为移动指令。真机桌面验收两张照片的采集分别耗时 11.5 秒、11.1 秒；第一张识别耗时 10.6 秒且结论为“无法判断”。这不是实时视频流，未完成实际导盲验证。
...
- `feat/glasses-demo-GPT` 的连续拍照入口是用户手动启动的独立 Demo，已在真机桌面场景验证两张图片的采集、页面刷新与自动识别链路；这两张桌面照片无法支持左/右结论。语音与清晰模拟通道下的左/右演示尚待录音和现场验收。
