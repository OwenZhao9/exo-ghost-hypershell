...
def parse_stream_line(line: str, host_t: float, cmd_l: float = 0.0, cmd_r: float = 0.0) -> Optional[Sample]:
    """解析一行 S: 帧；不是 S: 帧或字段数不对返回 None（FireWater 风格：按行、逗号分隔）"""
...
class ExoBridge:
    ...
    def __init__(self, port: Optional[str] = None, baud: int = BAUD,
        ...
        self.port = port
        self.baud = baud
        self.torque_limit = min(abs(torque_limit), FIRMWARE_LIMIT_NM)
        self.ramp_step = ramp_nm_per_s / send_hz          # 每个发送周期允许的最大变化
    ...
    def set_ramp(self, nm_per_s: float) -> None:
        ...
        self.ramp_step = max(0.05, float(nm_per_s)) * self.send_period
    ...
    def set_torque(self, left: float, right: float) -> None:
        ...
        l = max(-lim, min(lim, float(left)))
        r = max(-lim, min(lim, float(right)))
        with self._lock:
            self._target = (l, r)
    ...
    def _send_loop(self) -> None:
        ...
        while self.enabled and not self._stop.is_set():
            with self._lock:
                tl, tr = self._target
            cl, cr = self._current
            cl += max(-step, min(step, tl - cl))
            cr += max(-step, min(step, tr - cr))
            self._current = (cl, cr)
            try:
    ...
    def _handle_line(self, line: str) -> None:
        ...
        if line.startswith("S:"):
            ...
            if s.ldeg == 0.0 and s.rdeg == 0.0 and s.ldps == 0.0 and s.rdps == 0.0:
                ...
                self._zero_joint_frames = 0
            if self._csv_w:
                self._csv_w.writerow(asdict(s).values())
    ...
    def _open_log(self) -> None:
        if not self.log_dir or self._csv_w:
            return
        os.makedirs(self.log_dir, exist_ok=True)
        self.log_path = os.path.join(self.log_dir, time.strftime("session_%Y%m%d_%H%M%S.csv"))
        self._csv_f = open(self.log_path, "w", newline="", buffering=1 << 16)
        self._csv_w = csv.writer(self._csv_f)
        self._csv_w.writerow([f.name for f in fields(Sample)])
