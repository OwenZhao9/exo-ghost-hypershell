...
外骨骼腰带和双侧腿部固定环的视觉贴合参数位于 `data/twin-config.json` 的 `fit`。页面启动 3D 视图时收紧腰带与固定环，将固定环下沿抬到白膜人体膝关节上方约 1 厘米；腿部支撑杆下段平滑移到新环位，双侧组件在髋部运动时仍一起转动。配置读取不使用浏览器缓存。1 厘米由当前 GLB 骨骼与模型比例换算，未做人体实测；这些参数不代表实体绑带已收紧或适合穿戴。
